...
Процедура заполнитьДиаграммуПоИсточникамИнформации()
	...
	ЭтотОбъект.ДиаграммаПоРекламнымИсточникам.Обновление = Ложь;
	ЭтотОбъект.ДиаграммаПоРекламнымИсточникам.Очистить();

	Если источникИнформацииПокупок <> Неопределено Тогда
		точкаДиаграммыКоличество = ДиаграммаПоРекламнымИсточникам.Точки.Добавить("Количество");
		точкаДиаграммыКоличество.Текст = "Количество";
		точкаДиаграммыКоличество.ПриоритетЦвета = Ложь;

		Для Каждого элемент Из источникИнформацииПокупок Цикл
			источникИнформации_ = элемент.Ключ;
			количествоПокупок_ = элемент.Значение;

			серияДанныхИсточникиИнформации = ДиаграммаПоРекламнымИсточникам.Серии.Добавить(источникИнформации_);
			ЭтотОбъект.ДиаграммаПоРекламнымИсточникам.УстановитьЗначение(
				точкаДиаграммыКоличество, серияДанныхИсточникиИнформации, количествоПокупок_, Строка(количествоПокупок_));
		КонецЦикла;
	КонецЕсли;

	ЭтотОбъект.ДиаграммаПоРекламнымИсточникам.Обновление = Истина;
