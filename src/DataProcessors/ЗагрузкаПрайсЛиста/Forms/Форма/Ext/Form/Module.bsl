#Область ОбработчикиКомандФормы

&НаКлиенте
Асинх Процедура ЗагрузитьПрайсЛист(_)
    таблДок = Ждать получитьТабличныйДокументИзФайла();
КонецПроцедуры

#КонецОбласти // ОбработчикиКомандФормы

#Область СлужебныеПроцедурыИФункции

&НаКлиенте
Асинх Функция получитьТабличныйДокументИзФайла()
    результат = Неопределено;

    файлДанных = Ждать открытьТабличныйФайлДанных();
    Если файлДанных = Неопределено Тогда
        Возврат Неопределено; // Файл не выбран
    КонецЕсли;

    Попытка
        результат = получитьТабличныйДокументИзФайлаНаСервере(файлДанных.ИмяВременногоФайла);

    Исключение
        сообщение = Новый СообщениеПользователю;
        сообщение.Текст = СтрШаблон(
                "Ошибка чтения фала прайс-листа: ""%1"".
                |Описание ошибки: %2",
                файлДанных.ОписаниеПомещенногоФайла.СсылкаНаФайл.Имя,
                ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
        сообщение.Сообщить();

        результат = Неопределено;
    КонецПопытки;

    удалитьВременныйФайлНаСервере(файлДанных.ИмяВременногоФайла);

    Возврат результат;
КонецФункции

&НаКлиенте
Асинх Функция открытьТабличныйФайлДанных()
    параметрыВыбораФайла = Новый ПараметрыДиалогаПомещенияФайлов(
            "Выбор файла прайс-листа", Ложь, "Файл данных (*.xls, *.xlsx, *.ods)|*.xls;*.xlsx;*.ods");
    описаниеФайлаДанных = Ждать ПоместитьФайлНаСерверАсинх( , , "/home", параметрыВыбораФайла, ЭтотОбъект.УникальныйИдентификатор);

    Если описаниеФайлаДанных = Неопределено Тогда
        Возврат Неопределено; // Файл не выбран
    КонецЕсли;

    ДиагностикаКлиентСервер.Утверждение(описаниеФайлаДанных.ПомещениеФайлаОтменено = Ложь
    И описаниеФайлаДанных.Адрес <> Неопределено,
            "Файл должен быть помещен в хранилище.");

    // BSLLS:MissingTemporaryFileDeletion-off
    имяФайлаДанных = ПолучитьИмяВременногоФайла(описаниеФайлаДанных.СсылкаНаФайл.Расширение);
    потокДанных = ПолучитьИзВременногоХранилища(описаниеФайлаДанных.Адрес);
    потокДанных.Записать(имяФайлаДанных);

    Возврат Новый Структура("ИмяВременногоФайла, ОписаниеПомещенногоФайла, ПотокДанных", имяФайлаДанных, описаниеФайлаДанных, потокДанных);
КонецФункции

&НаСервереБезКонтекста
Функция получитьТабличныйДокументИзФайлаНаСервере(Знач имяФайлаДанных)
    результат = Новый ТабличныйДокумент;
    результат.Прочитать(имяФайлаДанных);

    Возврат результат;
КонецФункции

&НаСервереБезКонтекста
Функция удалитьВременныйФайлНаСервере(Знач имяФайлаДанных)
    Попытка
        УдалитьФайлы(имяФайлаДанных);
    Исключение
        записьОбОшибкеВЖурналРегистрацииНаСервере("ПолучитьТабличныйДокументИзФайла",
            ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));

        Возврат Ложь;
    КонецПопытки;

    Возврат Истина;
КонецФункции

&НаСервереБезКонтекста
Процедура записьОбОшибкеВЖурналРегистрацииНаСервере(Знач имяСобытия, Знач комментарий)
    ЗаписьЖурналаРегистрации(СтрШаблон("ЗагрузкаПрайсЛиста.%1", имяСобытия), УровеньЖурналаРегистрации.Ошибка, , ,
        ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
КонецПроцедуры

#КонецОбласти // СлужебныеПроцедурыИФункции
