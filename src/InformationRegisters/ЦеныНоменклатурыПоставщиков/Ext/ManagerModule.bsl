...
//  моментВремени - Дата, МоментВремени, Граница, Неопределено
// Возвращаемое значение:
//  - ТаблицаЗначений - Колонки таблицы: [Номенклатура, Цена]
...
Функция ПолучитьСписокЦенНоменклатуры(Знач списокНоменклатуры, Знач поставщикСсылка, Знач моментВремени = Неопределено) Экспорт
    параметрыОтбора = Новый Структура("МоментВремени, Номенклатура, Поставщик", моментВремени, номенклатура, поставщик);
    Возврат получитьДанныеРегистра("СрезПоследних",
            Новый ФиксированнаяСтруктура("Номенклатура, Цена"), параметрыОтбора, Истина);
КонецФункции
