...
Процедура установитьВидимостьПолейФормы()
	видыОперацийРасходаДенег = получитьВидыОперацийРасходаДенег();

	Элементы.ДоговорКонтрагента.Видимость = Истина;
	Элементы.Получатель.Видимость = НЕ (Объект.ВидОперации = видыОперацийРасходаДенег.ПрочийРасход);

	типПолучателя = Неопределено;
	Если Объект.ВидОперации = видыОперацийРасходаДенег.ПрочийРасход Тогда
		Элементы.ДоговорКонтрагента.Видимость = Ложь;

	ИначеЕсли Объект.ВидОперации = видыОперацийРасходаДенег.ВыдачаДенегПодотчетнику Тогда
		Элементы.ДоговорКонтрагента.Видимость = Ложь;
		типПолучателя = Тип("СправочникСсылка.Сотрудники");

	ИначеЕсли Объект.ВидОперации = видыОперацийРасходаДенег.ВзносНаличнымиВБанк Тогда
		Элементы.ДоговорКонтрагента.Видимость = Ложь;
		типПолучателя = Тип("СправочникСсылка.Банки");

	ИначеЕсли Объект.ВидОперации = видыОперацийРасходаДенег.ОплатаПоставщику
		ИЛИ Объект.ВидОперации = видыОперацийРасходаДенег.ВозвратДенегПокупателю Тогда

		типПолучателя = Тип("СправочникСсылка.Контрагенты");
		Элементы.ДоговорКонтрагента.Видимость = Объект.ВидОперации = видыОперацийРасходаДенег.ОплатаПоставщику;

		параметрыВыбора = Новый Массив;
		параметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.ТипКонтрагента",
				?(Объект.ВидОперации = видыОперацийРасходаДенег.ОплатаПоставщику,
		...
		Элементы.Получатель.ПараметрыВыбора = Новый ФиксированныйМассив(параметрыВыбора);
	КонецЕсли;
