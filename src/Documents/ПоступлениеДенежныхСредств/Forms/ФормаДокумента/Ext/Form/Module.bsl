...
Процедура ПриОткрытии(_)
	...
	Для Каждого поле Из поляИзменяющиеТипФормы Цикл
		применитьНастройкиПолейФормы(Новый Структура("ПолеИсточникСобытия", поле));
	КонецЦикла;
...
Процедура применитьНастройкиПолейФормы(настройкиПолей)
	...
	Если настройкиПолей.Свойство("ПолеИсточникСобытия") Тогда
		настройкиПолей = получитьНастройкиОтображенияПолей(настройкиПолей.ПолеИсточникСобытия);
	КонецЕсли;
...
Функция получитьТипПлательщика() // => Строка | Неопределено
	...
	Если Объект.ВидОперации = _ВидыОперацийПоступленияДенег.ВозвратОтПодотчетника Тогда
		строкаТипПлательщика = Тип("СправочникСсылка.Сотрудники");

	ИначеЕсли Объект.ВидОперации = _ВидыОперацийПоступленияДенег.ПолучениеНаличныхВБанке Тогда
		строкаТипПлательщика = Тип("СправочникСсылка.Банки");
	Иначе
		строкаТипПлательщика = Тип("СправочникСсылка.Контрагенты");
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура изменитьТипФормы(Знач полеИсточникСобытия)
	имяРеквизита = полеИсточникСобытия.Имя;

	Если НЕ проверитьПолеСостоянияФормыПоРеквизиту(имяРеквизита) Тогда
	...
	параметрыОтображенияПолей = получитьНастройкиОтображенияПолей(Элементы[имяРеквизита]);
	...
	Если поляДляОчистки.Количество() > 0 И ЗначениеЗаполнено(текстВопроса) Тогда
		...
		Если отменитьИзменение Тогда
			Объект[имяРеквизита] = _Состояние[имяРеквизита]; // Возврат значений полей (отмена очистки)
		Иначе
			очиститьНеиспользуемыеРеквизитыФормы(поляДляОчистки);
		КонецЕсли;
	КонецЕсли;

	_Состояние[имяРеквизита] = Объект[имяРеквизита];
...
// Параметры:
//	полеИсточникСобытия - ЭлементФормы - ВидОперации, ТипДенежныхСредств, Плательщик
...
&НаКлиенте
Функция создатьОбъектПараметровНастраиваемогоПоля(полеИсточникСобытия, скрываемыеПоля, изменяемыеПоля, типПлательщика = Неопределено)
	отображаемыеПоля = Новый Массив;
	Для Каждого поле Из изменяемыеПоля Цикл
		Если НЕ поле.Видимость И скрываемыеПоля.Получить(поле.Имя) = Неопределено Тогда
	...
	результат.Вставить("ПолеИсточникСобытия", полеИсточникСобытия);
