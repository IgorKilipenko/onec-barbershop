    ...
    Если этоНовыйДокументНаОснованииРТУ Тогда
        ...
        Если _Состояние.СтатусОплаты.ПризнакОплаты = _ПризнакиОплаты.ПолностьюОплачен Тогда
            ...
            сообщениеПользователю.Текст = СтрШаблон(
                    ...
                    _Состояние.СтатусОплаты.Представление);
...
&НаСервереБезКонтекста
Функция получитьСтатусОплатыНаСервере(Знач документРТУСсылка, Знач моментВремени = Неопределено)
    Возврат Документы.РеализацияТоваровИУслуг.ПроверитьСтатусОплаты(документРТУСсылка, моментВремени);
КонецФункции
...
Процедура инициализация()
    ...
    Если _Состояние.ЭтоОплатаНаОснованииДокументаРТУ Тогда
        моментВремени = ?(Объект.Проведен,
                получитьМоментВремениДокументаНаСервере(Объект.Ссылка, Объект.Дата, Ложь),
                Неопределено);
        _Состояние.СтатусОплаты = получитьСтатусОплатыНаСервере(Объект.ДокументОснование, моментВремени);
    КонецЕсли;
КонецПроцедуры
