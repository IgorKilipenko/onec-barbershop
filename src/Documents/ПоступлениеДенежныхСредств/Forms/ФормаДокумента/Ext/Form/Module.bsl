...
Процедура показатьЗапросПользователюНаОчисткуПолейФормы_ПриИзмененииТипаДенежныхСредств(обратныйВызов =
	...
	текстВопроса = СтрШаблон("При изменении реквизита ""%1"" будут очищены следующие данные:", имяРеквизита);
...
Процедура показатьЗапросПользователюНаОчисткуПолейФормы_ПриИзмененииВидаОперации(обратныйВызов = "_приЗапросеНаИзменениеВидаОперации_ОВ")
	...
	Если (значенияОчищаемыхПолейЗаполнены И (этоВозвратОтПодотчетника() ИЛИ этоПолучениеНаличныхВБанке())) Тогда
		...
		Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
			...
			текстВопроса = текстВопроса + "
				| - %1";
			текстВопроса = СтрШаблон(текстВопроса, "Договор");
		КонецЕсли;
		...
		Если этоПрочееПоступление() И ЗначениеЗаполнено(Объект.Плательщик) Тогда
			...
			текстВопроса = текстВопроса + "
				| - %1";
			текстВопроса = СтрШаблон(текстВопроса, "Плательщик");
		КонецЕсли;
...
Процедура показатьЗапросПользователюНаОчисткуПолейФормы_ПриИзмененииПлательщика(обратныйВызов = "_приЗапросеНаИзменениеПлательщика_ОВ")
	...
	Если этоКлиент И ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
		...
		текстВопроса = "При изменении реквизита ""%1"" будут очищены следующие данные:
			| - Договор
			| Продолжить?";

		текстВопроса = СтрШаблон(текстВопроса, имяРеквизита);
	КонецЕсли;
