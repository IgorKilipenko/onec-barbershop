...
//
Функция ПроверитьОплатуПоПоступлениюДС(Знач документРТУСсылка) Экспорт
...
Функция ПроверитьСтатусОплаты(Знач документРТУСсылка, моментВремени = Неопределено) Экспорт
    ...
    запрос.Текст =
        ...
        |	РеализацияТоваровИУслуг.Клиент КАК Клиент,
        |	ЕстьNULL(ВТ_Остатки.Контрагент, Неопределено) КАК Контрагент,
    ...
    ДиагностикаКлиентСервер.Утверждение(выборка.СуммаОстаток >= 0,
        "Сумма остатка задолженности не может иметь отрицательное значение.");
    ДиагностикаКлиентСервер.Утверждение(выборка.Клиент = выборка.Контрагент
            ИЛИ выборка.Контрагент = Неопределено И выборка.СуммаОстаток = 0,
