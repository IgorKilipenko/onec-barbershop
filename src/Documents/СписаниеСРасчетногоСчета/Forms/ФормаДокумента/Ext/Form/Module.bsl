...
&НаКлиенте
Перем _ВидыОперацийСписанияСРасчетногоСчета;
...
Процедура установитьНастройкиПоляПолучатель(Знач типПолучателя, Знач типКонтрагента = Неопределено)
    ДиагностикаКлиентСервер.Утверждение(
        типПолучателя = Тип("СправочникСсылка.Сотрудники")
            ...
            |ТипПолучателя должен принимать значение в диапазоне [СправочникСсылка.Сотрудники, СправочникСсылка.Контрагенты]",
            получитьКонтекстДиагностики("установитьНастройкиПоляПолучатель"));

    ДиагностикаКлиентСервер.Утверждение(
        типКонтрагента = Неопределено ИЛИ типКонтрагента = _ТипыКонтрагентов.Клиент ИЛИ типКонтрагента = _ТипыКонтрагентов.Поставщик,
            "Указанный ТипКонтрагента имеет недопустимое значение.
            |ТипКонтрагента должен принимать значение в диапазоне [ТипыКонтрагентов.Клиент, ТипыКонтрагентов.Поставщик]",
            получитьКонтекстДиагностики("установитьНастройкиПоляПолучатель"));
...
Функция получитьСкрываемыеПоляПриИзмененииВидаОперации()
    ...
    изменяемыеПоля.Добавить(Элементы.Получатель);
    ...
    Если ЗначениеЗаполнено(Объект.ВидОперации) Тогда
        Если этоСнятиеНаличныхВКассу() Тогда
            скрываемыеПоля.Вставить(Элементы.Получатель.Имя, Элементы.Получатель);
        Иначе
...
//  - Тип - Тип("СправочникСсылка.Сотрудники") если плательщиком должен быть Сотрудник.
//  - Неопределено - Есл тип Плательщика не определен для вида операции
&НаКлиенте
Функция определитьТипПолучателяПоВидуОперации()
    типКонтрагента = Неопределено;

    Если этоПеречислениеПодотчетномуЛицу() ИЛИ этоПеречислениеЗаработнойПлаты() Тогда
        Возврат Тип("СправочникСсылка.Сотрудники");

    ИначеЕсли этоОплатаПоставщику() Тогда
        типКонтрагента = _ТипыКонтрагентов.Поставщик;

    ИначеЕсли этоВозвратПокупателю() Тогда
...
Функция проверитьЭтоДопустимыйВидОперации(Знач видОперации)
    Возврат проверитьЭтоДопустимоеЗначениеУправляющегоПоля(видОперации, _ВидыОперацийСписанияСРасчетногоСчета);
КонецФункции
...
Функция этоОплатаПоставщику()
    Возврат Объект.ВидОперации = _ВидыОперацийСписанияСРасчетногоСчета.ОплатаПоставщику;
КонецФункции

// Возвращаемое значение:
//	- Булево
&НаКлиенте
Функция этоВозвратПокупателю()
    Возврат Объект.ВидОперации = _ВидыОперацийСписанияСРасчетногоСчета.ВозвратПокупателю;
КонецФункции

// Возвращаемое значение:
//	- Булево
&НаКлиенте
Функция этоСнятиеНаличныхВКассу()
    Возврат Объект.ВидОперации = _ВидыОперацийСписанияСРасчетногоСчета.СнятиеНаличныхВКассу;
КонецФункции

// Возвращаемое значение:
//	- Булево
&НаКлиенте
Функция этоПеречислениеПодотчетномуЛицу()
    Возврат Объект.ВидОперации = _ВидыОперацийСписанияСРасчетногоСчета.ПеречислениеПодотчетномуЛицу;
КонецФункции

// Возвращаемое значение:
//	- Булево
&НаКлиенте
Функция этоПеречислениеЗаработнойПлаты()
    Возврат Объект.ВидОперации = _ВидыОперацийСписанияСРасчетногоСчета.ПеречислениеЗаработнойПлаты;
КонецФункции
...
Процедура инициализация()
    ...
    _ВидыОперацийСписанияСРасчетногоСчета = значенияПеречислений.ВидыОперацийСписанияСРасчетногоСчета;
...
Процедура заполнитьПоляФормыЗначениямиПоУмолчанию(Знач принудительно = Ложь)
    Объект.ВидОперации = ?(НЕ принудительно И ЗначениеЗаполнено(Объект.ВидОперации),
            Объект.ВидОперации, _ВидыОперацийСписанияСРасчетногоСчета.ОплатаПоставщику);
КонецПроцедуры
...
Функция получитьЗначенияПеречислений()
    результат = Новый Структура;
    результат.Вставить("ВидыОперацийСписанияСРасчетногоСчета",
        Документы.СписаниеСРасчетногоСчета.ПолучитьВидыОперацийСписанияСРасчетногоСчета());
